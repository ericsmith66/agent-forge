...
1. **Security & Command Safety**
   ...
   - Do NOT use Rails system calls like `system()` or `%x` in any service that interacts with the Prefab API. Use `Open3` exclusively.
...
5. **Retry & Resilience**
   ...
   - Fail-Fast Clause: If a test fails 3 times consecutively, STOP and ask the user for an architectural review of the test strategy.
...
- [ ] Open3 used exclusively for shell/API calls (with timeouts)
- [ ] No backticks/system/exec anywhere (verified with grep)
- [ ] request_id (uuid) generated & stored on every attempt (PostgreSQL uuid type)
- [ ] source field present and populated
- [ ] Log scrubbing: sensitive API keys are filtered [FILTERED]
